...
class Coin:
    ...
    def moveCoin(self, event):
        ...
        if self.is_at_home():
            ...
            if check[0]:
                tkinter.messagebox.showinfo('INFO','You killed a coin! Now you get another chance.\nPlease Roll Dice Again')
                congrats = self.congratulations()
...
def startgame():
    ...
    start_label = tk.Label(ludo.get_frame(), text="Let's Kick Off! \n Let\'s Begin with {}".format(turn[0]), font=(None, 20),
                         width=30, height=3, borderwidth=3, relief=tk.SUNKEN)
    start_label.place(x=40, y=100)
    top.destroy()
...
welcome_msg = ''' Welcome into the game of LUDO :-) \n
        Rules of the game:
